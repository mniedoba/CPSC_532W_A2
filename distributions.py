...
if scaling_function == 'softplus':
    ...
    positive_function = torch.exp
    positive_inverse = torch.log
else:
...
class Exponential(torch.distributions.Exponential):
...
class Bernoulli(torch.distributions.Bernoulli):
    ...
    def __init__(self, probs=None, logits=None):
        if logits is None and probs is None:
            ...
            logits = torch.log(probs/(1.-probs)) # NOTE: This will fail if probs = 0
...
class DistributionCollection():
    ...
    def __init__(self, component_distributions):
        self.component_distributions = component_distributions

    def sample(self, sample_shape=None):
        samples = []
        for component_distribution in self.component_distributions:
            sample = component_distribution.sample(sample_shape)
            samples.append(sample)
        return samples

    def log_prob(self, values):
        assert len(values) == len(self.component_distributions)
        log_probs = torch.zeros(values[0].shape[0])
        for value, dist in zip(values, self.component_distributions):
    ...
    def optim_params(self):
        params = []
        for dist in self.component_distributions:
            params += dist.optim_params()
        return params

    def params(self):
        all_params = []
        for dist in self.component_distributions:
            all_params += dist.params()
        return all_params
...
if __name__ == '__main__':
    ...
    # Prior parameters
    prior_loc = torch.tensor(1.)
    prior_scale = torch.sqrt(torch.tensor(5.))

    # Likelihood parameters
    likelihood_scale = torch.sqrt(torch.tensor(2.))
    ...
    # Q distribution
    Q_loc, Q_scale = torch.clone(prior_loc), torch.clone(prior_scale) # NOTE: Clone is necessary here
    ...
    optimizer = torch.optim.Adam([Q_loc, Q_optim_scale], lr=learning_rate)
    print('Initial Q:', Q)
    for _ in range(num_steps):
        ...
        log_likelihood = 0.
        for y in ys:
            log_likelihood += torch.distributions.Normal(loc=Q_samples, scale=likelihood_scale).log_prob(y)
        ...
        optimizer.step()
        print('Q:', Q, 'log_P', log_P.mean(), 'ELBO', ELBO_loss, 'log_W', log_W)
